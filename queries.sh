...
echo "$($PSQL "select avg(winner_goals + opponent_goals) from games")" #fix this!!!!!!!!!
...
echo "$($PSQL "select distinct(name) from teams full join games on teams.team_id = games.winner_id where winner_id = team_id order by name")"
